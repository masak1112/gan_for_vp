...
import os
from avg_loss_functions import combined_loss
from avg_tfutils import w, b
from avg_utils import psnr_error, sharp_diff_error

# noinspection PyShadowingNames
class GeneratorModel:
    def __init__(self, session, summary_writer, height_train, width_train, height_test,
                 width_test, scale_layer_fms, scale_kernel_sizes,padding_g, adversarial=None,
                 context_frames=10,sequence_length=20,lr_g=0.0004):
        """
        ...
        with tf.name_scope('generator'):
            ...
            with tf.name_scope('data'):
                self.input_frames_train = tf.placeholder(
                    tf.float32, shape=[None, self.height_train, self.width_train, 3 * self.context_frames])
                ...
                self.input_frames_test = tf.placeholder(
                    tf.float32, shape=[None, self.height_test, self.width_test, 3 * self.context_frames])
            ...
            with tf.name_scope('train'):
                ...
                self.optimizer = tf.train.AdamOptimizer(learning_rate=self.lr_g, name='optimizer')
    ...
    def train_step(self, batch, discriminator=None):
        ...
        @param batch: An array of shape
                      [c.BATCH_SIZE x self.height x self.width x (3 * (self.context_frames + 1))].
        ...
        if global_step % c.IMG_SAVE_FREQ == 0:
            ...
            for pred_num in range(len(input_frames)):
                ...
                # save input images
                for frame_num in range(self.context_frames):
    ...
    def test_batch(self, batch, global_step, num_rec_out=1, save_imgs=True):
        ...
        @param batch: An array of shape
                      [batch_size x self.height x self.width x (3 * (self.context_frames+ num_rec_out))].
        ...
        input_frames = batch[:, :, :, :3 * self.context_frames]
        gt_frames = batch[:, :, :, 3 * self.context_frames:]
